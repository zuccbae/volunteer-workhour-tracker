document.addEventListener('DOMContentLoaded', () => {
  ...
  function getWeekNumber() {
    ...
    today.setDate(today.getDate() - dayNum + 3);
    const firstThursday = new Date(today.getFullYear(), 0, 4);
    const weekNum = Math.floor((today - firstThursday) / (7 * 24 * 60 * 60 * 1000)) + 1;
  ...
  function renderGroupedEntries(grouped) {
    ...
    for (const week in grouped) {
      ...
      grouped[week].forEach(entry => {
        ...
        li.textContent = `${entry["First Name"]} ${entry["Last Name"]} – ${entry.Month} ${entry.Year} – ${entry["Number of Days Spent"]} day(s) – ${entry["Task Type"]}`;
        ul.appendChild(li);
      });
